...
public class SingleFireTower : Tower
{
    ...
    public void SingelDestroy(float amountDamage)
    {
        foreach (GameObject enemies in targetList)
        {
            if(cooldown<= 0)
            {
                Vector3 rains = enemies.transform.position + (enemies.transform.forward * enemies.GetComponent<TestWay>().movespeed * UIManager.gameSpeed) + new Vector3(0f, airAmount, 0f);
